...
console.log(add2(constant(1), constant(2))); // 3
...
console.log(addn([constant(3), constant(7), one, two, constant(11)])); // 24
  ...
  addn(
    vals
      .reduce(function reducer(a, v) {
        // if (!a.includes(v)) return a.concat(v);
        if (!a.includes(v)) return [...a, v];
        return a;
      }, [])
